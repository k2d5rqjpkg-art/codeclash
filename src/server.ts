...
async function handleRequest(req: http.IncomingMessage, res: http.ServerResponse) {
  ...
  // ── AI Code Generation ──
  // POST /api/tanks/generate — multi-model support
  if (method === "POST" && url.pathname === "/api/tanks/generate") {
    const b = await body(req);
    const prompt = b.prompt;
    const apiKey = b.apiKey;
    const model = b.model || "anthropic";
    const skill = b.skill || "shield";
    if (!prompt) { error(res, "prompt required"); return; }
    if (!apiKey) { error(res, "apiKey required"); return; }
...
- 7 Skills: shield(block)/sprint(double speed)/cloak(invisible)/freeze(stun enemy)/stun(randomize)/poison(slow)/teleport(instant move)
- Center capture point: hold solo 120 frames to win
- Enemy is null when hidden (grass/cloak/out of 90° vision cone)
- Keep code under 80 lines, handle all edge cases`;

    const userPrompt = `Player request: "${prompt}". Agent skill: ${skill}. Write the strategy function.`;
      ...
      switch (model) {
        case "anthropic": {
          const resp = await fetch("https://api.anthropic.com/v1/messages", {
            ...
            body: JSON.stringify({ model: "claude-haiku-4-5", max_tokens: 1500, system, messages: [{ role: "user", content: userPrompt }] })
          });
        ...
        case "openai": {
          const resp = await fetch("https://api.openai.com/v1/chat/completions", {
            ...
            body: JSON.stringify({ model: "gpt-4o-mini", max_tokens: 1500, temperature: 0.8, messages: [{ role: "system", content: system }, { role: "user", content: userPrompt }] })
          });
          const data: any = await resp.json();
        ...
        case "deepseek": {
          const resp = await fetch("https://api.deepseek.com/v1/chat/completions", {
            method: "POST", headers: { "Content-Type": "application/json", "Authorization": "Bearer " + apiKey },
            body: JSON.stringify({ model: "deepseek-chat", max_tokens: 1500, temperature: 0.8, messages: [{ role: "system", content: system }, { role: "user", content: userPrompt }] })
          });
          const data: any = await resp.json();
          code = data.choices?.[0]?.message?.content || "";
        ...
        case "gemini": {
          const resp = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${apiKey}`, {
            ...
            body: JSON.stringify({ system_instruction: { parts: [{ text: system }] }, contents: [{ parts: [{ text: userPrompt }] }], generationConfig: { maxOutputTokens: 1500, temperature: 0.8 } })
          });
          const data: any = await resp.json();
        ...
        default:
          return error(res, `Unknown model: ${model}. Use: anthropic, openai, deepseek, gemini`);
      }
      code = extractFn(code);
      json(res, { code, skill, model });
    } catch (err: any) {
